...
public class FruitPress extends BlockContainer {
	...
	public FruitPress(){
		...
		this.setBlockBounds(-0.5F, 0F, -0.5F, 1.5F, 2F, 1.5F);
   ...
   public static int getDirection(int side){
	...
	public Item getItemDropped(int i, Random random, int j){
		return Item.getItemFromBlock(ShopKeeper.machinaFruitPress);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ){
		if(world.isRemote){
			...
			FMLNetworkHandler.openGui(player, AceCraft.instance, ShopKeeper.guiIDFruitPress, world, x, y, z);
			return true;
		}
		return false;
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityplayer, ItemStack itemstack){
		...
		int e = 0;
		if(world.getBlock(x, y+2, z) == ShopKeeper.machinaAxle){
			TileEntityAxle entityA = (TileEntityAxle) world.getTileEntity(x, y+2, z);
			e = entityA.energy;
		}
		if(world.getBlock(x, y+2, z) == ShopKeeper.machinaGearBox){
			TileEntityGearBox entityG = (TileEntityGearBox) world.getTileEntity(x, y+2, z);
			e = entityG.energy;
		}
		((TileEntityFruitPress)world.getTileEntity(x, y, z)).firstSetUp(l, e);
	}
	...
	public void breakBlock(World world, int x, int y, int z, Block oldblock, int oldMetadata){
		if(!keepInventory){
			TileEntityFruitPress tileentity = (TileEntityFruitPress) world.getTileEntity(x, y, z);
			if(tileentity != null){
