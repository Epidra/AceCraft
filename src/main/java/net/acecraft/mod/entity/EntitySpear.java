...
public class EntitySpear extends EntityArrow implements IProjectile {
    ...
    public EntitySpear(World worldIn){
        ...
        this.yTile = -1;
        this.zTile = -1;
        this.canBePickedUp = PickupStatus.DISALLOWED;
        this.damage = 2.0D;
    ...
    public EntitySpear(World world, EntityPlayer entity, float value, String _id, float dmg){
		this(world, entity.posX, entity.posY + (double)entity.getEyeHeight() - 0.10000000149011612D, entity.posZ);
        ...
        if (entity != null){
            ...
            if (entity.attackEntityFrom(damagesource, (float)i)){
                ...
                if (!this.worldObj.isRemote && this.motionX * this.motionX + this.motionY * this.motionY + this.motionZ * this.motionZ < 0.0010000000474974513D){
                    if (this.canBePickedUp == PickupStatus.ALLOWED){
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund){
    	this.id = tagCompund.getString("id");
        ...
        if (tagCompund.hasKey("pickup", 99)){
            this.canBePickedUp = PickupStatus.func_188795_a(tagCompund.getByte("pickup"));
        } else if (tagCompund.hasKey("player", 99)){
            this.canBePickedUp = tagCompund.getBoolean("player") ? PickupStatus.ALLOWED : PickupStatus.DISALLOWED;
        }
    }
    
    public void onCollideWithPlayer(EntityPlayer player){
        if (!this.worldObj.isRemote && this.inGround){
        	boolean flag = this.canBePickedUp == PickupStatus.ALLOWED || this.canBePickedUp == PickupStatus.CREATIVE_ONLY && player.capabilities.isCreativeMode;
            ItemStack stack;
           /*      if(id.compareTo("ToolSpearFlint")      == 0) { stack = new ItemStack(ShopKeeper.toolSpearFlint,      1); }
            ...
            if (this.canBePickedUp == PickupStatus.ALLOWED && !player.inventory.addItemStackToInventory(stack)){
                flag = false;
            }
            if (flag){
    ...
    protected ItemStack getArrowStack() {
    	/*     if(id.compareTo("ToolSpearFlint")      == 0) { return new ItemStack(ShopKeeper.toolSpearFlint,      1); }
    	else if(id.compareTo("ToolSpearObsidian")   == 0) { return new ItemStack(ShopKeeper.toolSpearObsidian,   1); }
        else if(id.compareTo("ToolSpearIron")       == 0) { return new ItemStack(ShopKeeper.toolSpearIron,       1); }
        else if(id.compareTo("ToolSpearGold")       == 0) { return new ItemStack(ShopKeeper.toolSpearGold,       1); }
        else if(id.compareTo("ToolSpearCopper")     == 0) { return new ItemStack(ShopKeeper.toolSpearCopper,     1); }
        else if(id.compareTo("ToolSpearBronze")     == 0) { return new ItemStack(ShopKeeper.toolSpearBronze,     1); }
        else if(id.compareTo("ToolSpearSteel")      == 0) { return new ItemStack(ShopKeeper.toolSpearSteel,      1); }
        else if(id.compareTo("ToolSpearMythril")    == 0) { return new ItemStack(ShopKeeper.toolSpearMythril,    1); }
        else if(id.compareTo("ToolSpearIridium")    == 0) { return new ItemStack(ShopKeeper.toolSpearIridium,    1); }
        else if(id.compareTo("ToolSpearAdamantium") == 0) { return new ItemStack(ShopKeeper.toolSpearAdamantium, 1); }
        else if(id.compareTo("ToolSpearUnobtanium") == 0) { return new ItemStack(ShopKeeper.toolSpearUnobtanium, 1); }
        else{*/ return new ItemStack(Items.FLINT, 1); //}
	}
