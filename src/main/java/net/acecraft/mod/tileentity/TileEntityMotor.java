...
public class TileEntityMotor extends TileEntity {
    ...
    private void updateEnergy(int vX, int vY, int vZ){
    	if(worldObj.getBlock(xCoord+vX, yCoord+vY, zCoord+vZ).equals(ShopKeeper.machinaCable)){
    		TileEntityCable entityC = (TileEntityCable) worldObj.getTileEntity(xCoord+vX, yCoord+vY, zCoord+vZ);
    		energy = entityC.energyCollected;
    	}else{
    		energy = 0;
    	}
    }
