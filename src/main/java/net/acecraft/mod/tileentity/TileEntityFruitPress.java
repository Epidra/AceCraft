...
public class TileEntityFruitPress extends ITileEntityEnergy implements ISidedInventory{
    ...
    public void update(){
        boolean flag1 = false;
        if(worldObj.getBlockState(pos.up(2)).getBlock() == ShopKeeper.machinaAxle){
			TileEntityAxle entityA = (TileEntityAxle) worldObj.getTileEntity(pos.up(2));
			energy = entityA.energy;
		}else
		if(worldObj.getBlockState(pos.up(2)).getBlock() == ShopKeeper.machinaGearBox){
			TileEntityGearBox entityG = (TileEntityGearBox) worldObj.getTileEntity(pos.up(2));
    ...
    private boolean canSmelt(){
        if (this.itemStacks[0] == null){
            ...
            if((0 == content.compareTo("Cherry")    || 0 == content.compareTo("empty")) && fillingLevel < 25 && this.itemStacks[0].getItem() == ShopKeeper.foodCherries)                            return true;
    ...
    public void smeltItem(){
        if (this.canSmelt()){
            ...
            if(this.itemStacks[0].getItem() == ShopKeeper.foodCherries)                           { fillingLevel++; if(0 == content.compareTo("empty")) content = "Cherry"; }
