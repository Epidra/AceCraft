...
public class ITileEntityEnergy extends TileEntity implements ITickable {
    ...
    public void update(){
    	if(facing == null){
    		facing = worldObj.getBlockState(pos).getValue(IMachinaEnergy.FACING).getOpposite();
    	} else {
    		if(worldObj.getBlockState(pos.offset(facing)).getBlock() instanceof IMachinaEnergy){
    			ITileEntityEnergy entity = (ITileEntityEnergy) worldObj.getTileEntity(pos.offset(facing));
