...
public class ContainerAnvilRepair extends Container
{
    ...
    public ContainerAnvilRepair(InventoryPlayer playerInventory, World worldIn, EntityPlayer player){
        this(playerInventory, worldIn, BlockPos.ORIGIN, player);
    }
    
    public ContainerAnvilRepair(InventoryPlayer playerInventory, final World worldIn, final BlockPos blockPosIn, EntityPlayer player){
        ...
        this.addSlotToContainer(new Slot(this.outputSlot, 2, 134, 47){
            ...
            public boolean canTakeStack(EntityPlayer playerIn){
                return (playerIn.capabilities.isCreativeMode || playerIn.experienceLevel >= ContainerAnvilRepair.this.maximumCost) && ContainerAnvilRepair.this.maximumCost > 0 && this.getHasStack();
            }
            public void onPickupFromSlot(EntityPlayer playerIn, ItemStack stack){
                if (!playerIn.capabilities.isCreativeMode){
                    playerIn.addExperienceLevel(-ContainerAnvilRepair.this.maximumCost);
                }
                float breakChance = net.minecraftforge.common.ForgeHooks.onAnvilRepair(playerIn, stack, ContainerAnvilRepair.this.inputSlots.getStackInSlot(0), ContainerAnvilRepair.this.inputSlots.getStackInSlot(1));
                ContainerAnvilRepair.this.inputSlots.setInventorySlotContents(0, (ItemStack)null);
                if (ContainerAnvilRepair.this.materialCost > 0){
                    ItemStack itemstack = ContainerAnvilRepair.this.inputSlots.getStackInSlot(1);
                    if (itemstack != null && itemstack.stackSize > ContainerAnvilRepair.this.materialCost){
                        itemstack.stackSize -= ContainerAnvilRepair.this.materialCost;
                        ContainerAnvilRepair.this.inputSlots.setInventorySlotContents(1, itemstack);
                    } else {
    ...
    public void updateRepairOutput(){
        ...
        if (itemstack == null){
            ...
            boolean flag = false;
            if (itemstack2 != null){
                if (!onAceAnvilChange(this, itemstack, itemstack2, outputSlot, repairedItemName, j)) return;
    ...
    public boolean canInteractWith(EntityPlayer playerIn){
        return this.theWorld.getBlockState(this.selfPosition).getBlock() != ShopKeeper.machinaAnvil ? false : playerIn.getDistanceSq((double)this.selfPosition.getX() + 0.5D, (double)this.selfPosition.getY() + 0.5D, (double)this.selfPosition.getZ() + 0.5D) <= 64.0D;
    }
    ...
    public static boolean onAceAnvilChange(ContainerAnvilRepair container, ItemStack left, ItemStack right, IInventory outputSlot, String name, int baseCost){
