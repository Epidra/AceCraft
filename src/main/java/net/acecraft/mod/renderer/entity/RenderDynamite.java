...
public class RenderDynamite extends Render {
	private Item fielItem;
    private int fieldInteger;
    
    public RenderDynamite(Item item, int i){
        this.fielItem = item;
        this.fieldInteger = i;
    }
    
    public RenderDynamite(Item item){
        this(item, 0);
    }
    
    public void doRender(Entity entity, double dX, double dY, double dZ, float fA, float fB){
        IIcon iicon = this.fielItem.getIconFromDamage(this.fieldInteger);
        if (iicon != null){
            GL11.glPushMatrix();
