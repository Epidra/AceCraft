...
public class IMachinaDoubleWide extends BlockContainer {
	...
	/** Default Constructor */
	public IMachinaDoubleWide(String name, Material material) {
		super(material);
   ...
   public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack){
       ...
       if(placer.getHorizontalFacing().getOpposite() == EnumFacing.NORTH) {
    	   if(worldIn.isAirBlock(pos.west())) {
    		   worldIn.setBlockState(pos.west(), state.withProperty(FACING, placer.getHorizontalFacing().getOpposite()).withProperty(PRIMARY, Boolean.valueOf(false)), 2);
    	   } else {
       ...
       if(placer.getHorizontalFacing().getOpposite() == EnumFacing.SOUTH) {
    	   if(worldIn.isAirBlock(pos.east())) {
    		   worldIn.setBlockState(pos.east(), state.withProperty(FACING, placer.getHorizontalFacing().getOpposite()).withProperty(PRIMARY, Boolean.valueOf(false)), 2);
    	   } else {
    		   worldIn.destroyBlock(pos, true);
    	   }
       }
       if(placer.getHorizontalFacing().getOpposite() == EnumFacing.WEST ) {
    	   if(worldIn.isAirBlock(pos.south())) {
    		   worldIn.setBlockState(pos.south(),  state.withProperty(FACING, placer.getHorizontalFacing().getOpposite()).withProperty(PRIMARY, Boolean.valueOf(false)), 2);
    	   } else {
    		   worldIn.destroyBlock(pos, true);
    	   }
       }
       if(placer.getHorizontalFacing().getOpposite() == EnumFacing.EAST ) {
    	   if(worldIn.isAirBlock(pos.north())) {
    		   worldIn.setBlockState(pos.north(),  state.withProperty(FACING, placer.getHorizontalFacing().getOpposite()).withProperty(PRIMARY, Boolean.valueOf(false)), 2);
    	   } else {
    		   worldIn.destroyBlock(pos, true);
    	   }
       }
   ...
   public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player){
	   ...
	   if(enumfacing == EnumFacing.NORTH) worldIn.destroyBlock(pos.west(),  true);
	   if(enumfacing == EnumFacing.SOUTH) worldIn.destroyBlock(pos.east(),  true);
	   if(enumfacing == EnumFacing.EAST ) worldIn.destroyBlock(pos.north(), true);
	   if(enumfacing == EnumFacing.WEST ) worldIn.destroyBlock(pos.south(), true);
   }
   ...
   public void onBlockExploded(World world, BlockPos pos, Explosion explosion){
	   ...
	   if(!isPrimary) enumfacing = enumfacing.getOpposite();
	   if(enumfacing == EnumFacing.NORTH) world.destroyBlock(pos.west(),  true);
	   if(enumfacing == EnumFacing.SOUTH) world.destroyBlock(pos.east(),  true);
	   if(enumfacing == EnumFacing.EAST ) world.destroyBlock(pos.north(), true);
	   if(enumfacing == EnumFacing.WEST ) world.destroyBlock(pos.south(), true);
   ...
   //----------------------------------------SUPPORT----------------------------------------//
    ...
    public IBlockState getStateFromMeta(int meta){
        EnumFacing enumfacing = EnumFacing.byHorizontalIndex(meta % 8);
