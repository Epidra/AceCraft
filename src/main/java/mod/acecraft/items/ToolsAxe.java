 ...
 * @author derf6060
 */
public class ToolsAxe extends ItemTool {
	 ...
	 */
	public ToolsAxe(String name, ToolMaterial material) {
	...
	private boolean checkStrVsBlock(ItemStack stack, IBlockState state) {
		...
		return b || 
			   ...
			   ((state.getBlock().getHarvestTool(state) != null && state.getBlock().getHarvestTool(state).equals("axe")) && state.getBlock().getHarvestLevel(state) <= this.material.getHarvestLevel());
	}
