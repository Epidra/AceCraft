...
public class ContainerCookingBoard extends ContainerBase {
    ...
    public ContainerCookingBoard(ContainerType<?> type, int windowID, PlayerInventory playerInventory, PacketBuffer packetBuffer) {
    ...
    public ContainerCookingBoard(ContainerType<?> type, int windowID, PlayerInventory playerInventory, BlockPos pos) {
        this(type, windowID, playerInventory, (TileBase) playerInventory.player.getEntityWorld().getTileEntity(pos));
        //this.pos = pos;
    }

    public ContainerCookingBoard(int windowID, PlayerInventory playerInventory, TileBase board) {
        this(ShopKeeper.CONTAINER_COOKINGBOARD.get(), windowID, playerInventory, board);
    }

    public ContainerCookingBoard(int windowID, PlayerInventory playerInventory, PacketBuffer packetBuffer) { // Forge Registry
        this(ShopKeeper.CONTAINER_COOKINGBOARD.get(), windowID, playerInventory, packetBuffer);
    }

    public ContainerCookingBoard(ContainerType<?> type, int windowID, PlayerInventory playerInventory, TileBase board) {
        super(type, windowID);
    ...
    protected static void func_217066_a(int ID, World world, PlayerEntity player, CraftingInventory craftingInventory, CraftResultInventory craftResult) {
        if (!world.isRemote) {
            ...
            Optional<RecipeCookingBoard> optional = world.getServer().getRecipeManager().getRecipe(RecipeCookingBoard.stove, craftingInventory, world);
            if (optional.isPresent()) {
                RecipeCookingBoard icraftingrecipe = optional.get();
