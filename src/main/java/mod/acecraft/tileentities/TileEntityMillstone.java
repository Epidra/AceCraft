...
import mod.acecraft.crafting.MillstoneRecipes;
...
public class TileEntityMillstone extends ITileEntityEnergy implements ITickable, ISidedInventory {
    ...
    public void update(){
        ...
        boolean flag1 = false;
        if(world.getBlockState(pos.up(3)).getBlock() == ShopKeeper.MACHINA_AXLE){
			TileEntityAxle entityA = (TileEntityAxle) world.getTileEntity(pos.up(3));
			energy = entityA.energy;
		}else
		if(world.getBlockState(pos.up(3)).getBlock() == ShopKeeper.MACHINA_GEARBOX){
			TileEntityGearBox entityG = (TileEntityGearBox) world.getTileEntity(pos.up(3));
    ...
    private boolean canSmelt(){
    	if (this.itemStacks.get(0) == null){
            ...
            ItemStack itemstack = MillstoneRecipes.instance().getSmeltingResult(this.itemStacks.get(0));
            if (itemstack == null) return false;
            if (this.itemStacks.get(1) == null) return true;
            if (!this.itemStacks.get(1).isItemEqual(itemstack)) return false;
            int result = itemStacks.get(1).getCount() + itemstack.getCount();
            return result <= getInventoryStackLimit() && result <= this.itemStacks.get(1).getMaxStackSize(); //Forge BugFix: Make it respect stack sizes properly.
        }
    }
    ...
    public void smeltItem(){
    	if (this.canSmelt()){
            ItemStack itemstack = MillstoneRecipes.instance().getSmeltingResult(this.itemStacks.get(0));
            if (this.itemStacks.get(1) == null){
                this.itemStacks.set(1, itemstack);
            }else if (this.itemStacks.get(1).getItem() == itemstack.getItem()){
                this.itemStacks.get(1).grow(itemstack.getCount());
            }
