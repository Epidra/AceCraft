...
public class EntityAlpaca extends Animal implements Shearable {
	...
	private static final Map<DyeColor, Integer> COLOR_BY_DYE = Maps.<DyeColor, Integer>newEnumMap(
			Arrays.stream(DyeColor.values()).collect(Collectors.toMap(p_29868_ -> (DyeColor)p_29868_, EntityAlpaca::createSheepColor))
	);
	...
	public EntityAlpaca(EntityType<? extends EntityAlpaca> entityType, Level level) {
	...
	@Nullable
	public EntityAlpaca getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
		EntityAlpaca sheep = Register.ENTITY_ALPACA.get().create(level);
		if (sheep != null) {
			sheep.setColor(this.getOffspringColor(this, (EntityAlpaca)otherParent));
		}
	...
	private DyeColor getOffspringColor(Animal father, Animal mother) {
		DyeColor dyecolor = ((EntityAlpaca)father).getColor();
		DyeColor dyecolor1 = ((EntityAlpaca)mother).getColor();
